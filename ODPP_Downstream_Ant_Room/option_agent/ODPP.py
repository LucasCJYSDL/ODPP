...
class ODPP_Agent(Option_Agent):
    def __init__(self, args):
        ...
        # self.dec_func = rnn_decoder.RNN_Decoder(input_dim=2, hidden_dim=self.args.rnn_dec_hidden_dim,
        #                                        code_dim=self.args.code_dim)  # take traj as input
    ...
    def train(self, episode_id, train_batch, lap_repr: LapReprLearner):
        # set up the data
        ...
        for _ in range(self.args.dec_iters):
            _, log_gt, _ = self.dec_func.forward(landmarks[:, :, :2], gt=options.squeeze(-1))  # (bs, )
        ...
        # preparation
        _, dec_rwd, _ = self.dec_func.forward(landmarks[:, :, :2], gt=options.squeeze(-1))  # (bs, )
