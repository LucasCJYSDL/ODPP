...
class LapReprLearner:
    ...
    def __init__(self, common_args, env):
        ...
        self.env = env
        # NN
        self._repr_fn = MLP(layers=[2]+[self.args.lap_n_units for _ in range(self.args.lap_n_layers)]+[self.args.d],
    ...
    def load_models(self, path):
        ...
        self._repr_fn.load_state_dict(torch.load(path, map_location=lambda storage, loc: storage))
    ...
    def _build_loss(self, batch): # modification
        ...
        s2 = batch['s2']
        s_neg = batch['s_neg']
        s1_repr = self._repr_fn(s1[:, :2])
        s2_repr = self._repr_fn(s2[:, :2])
        s_neg_repr = self._repr_fn(s_neg[:, :2])
    ...
    def get_embedding_matrix(self, obs_input, normalized=True):
        ...
        with torch.no_grad():
            raw_embedding_segment = self._repr_fn(obs_input[:, :2])
    ...
    def visualize_embeddings(self):
        ...
        with torch.no_grad():  # danger
            ...
            while cur_idx < data_size:
                ...
                raw_embedding_segment = self._repr_fn(data_segment[:, :2])
        ...
        plt.scatter(x=axis_x, y=axis_y, c=value, cmap="viridis", alpha=0.3)
        plt.colorbar()
        plt.savefig(dir + '/' + 'embedding_{}.png'.format(1))
