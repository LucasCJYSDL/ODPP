...
class Runner(object):
    ...
    def _rollout(self, episode_id):
        ...
        for rollout_idx in tqdm(range(rollout_num)):
            ...
            self._traj_rollout(episode_batch, c_onehot, s)
    ...
    def _rollout_batch(self, episode_id):
        ...
        while traj_idx < self.args.traj_num:
            ...
            for c_id in range(self.args.batch_c):
    ...
    def _draw_option_trajectories(self, epi_idx, landmark_only=False):
        ...
        for c_id in tqdm(range(self.args.code_dim)):
            ...
            x_list.append(raw_traj_x[-1])
            y_list.append(raw_traj_y[-1])
            dist_list.append((raw_traj_x[-1]**2 + raw_traj_y[-1]**2)**0.5)
        ...
        return np.mean(dist_list), np.std(x_list), np.std(y_list)
    ...
    def train(self):
        ...
        for idx in range(10):
            ...
            if self.buffer.can_sample(self.args.traj_num):
                ...
                if self.args.agent_id == 'ODPP' or self.args.agent_id == 'DCO':
                    log_info = self.agent.train(idx, train_batch, self.lap_repr)
                else:
                    self.agent.train(idx, train_batch)

            f, g, h = self._draw_option_trajectories(idx, landmark_only=True)
        ...
        mean_confidence_interval(f_list)
        mean_confidence_interval(g_list)
        mean_confidence_interval(h_list)
