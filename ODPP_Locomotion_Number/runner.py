    ...
    def _get_traj_stat(self):
        ...
        for c_id in tqdm(range(self.args.code_dim)):
            ...
            for traj_id in range(10):
                ...
                c_traj_rwd_list.append(c_traj_rwd)
            traj_rwd_list.append(np.mean(c_traj_rwd_list))

        return {'traj_rwd_mean': np.mean(traj_rwd_list), 'traj_rwd_std': np.std(traj_rwd_list)}
        ...
        for idx in range(self.args.episode_num):
            ...
            if self.buffer.can_sample(self.args.traj_num):
                ...
                if self.args.agent_id == 'ODPP' or self.args.agent_id == 'DCO':
                    self.agent.train(idx, train_batch, self.lap_repr, traj_rwd=traj_stat)
                else:
                    self.agent.train(idx, train_batch, traj_rwd=traj_stat)
            # save model
